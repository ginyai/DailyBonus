...
public class BonusRequirementOnlineTimeTotal implements BonusRequirement {
    private final DailyBonusMain dailyBonus;
    private final long requirement;

    public BonusRequirementOnlineTimeTotal(DailyBonusMain dailyBonus, long requirement) {
    ...
    public boolean check(PlayerData player) {
        return player.getOnlineTimeTotal() > requirement;
    }
    ...
    public Text format(Player player) {
        ...
        return dailyBonus.getI18n()
            .translateToLocal("requirement.online_time_total.format", ImmutableMap.of(
